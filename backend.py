    ...
    def do_GET(self):
        ...
        with db() as conn:
            ...
            if path == "/api/admin/dashboard":
                ...
                tomorrow = conn.execute("SELECT b.*, s.name service_name, s.price_uzs, u.name customer_name, u.email customer_email, u.phone customer_phone FROM bookings b JOIN services s ON s.id=b.service_id JOIN users u ON u.id=b.user_id WHERE date(b.starts_at)=date('now','+1 day') AND b.status != 'CANCELLED' ORDER BY b.starts_at ASC").fetchall()
                visits = conn.execute("SELECT COUNT(*) FROM site_visits WHERE visit_day >= date('now', ?)", (start,)).fetchone()[0]
                unique_today = conn.execute("SELECT COUNT(*) FROM site_visits WHERE visit_day=date('now')").fetchone()[0]
                return self.json(200, {"period": period, "summary": dict(totals), "visits": visits, "unique_today": unique_today, "services": [dict(x) for x in services], "bookings": [dict(x) for x in rows], "upcoming": [dict(x) for x in upcoming], "tomorrow": [dict(x) for x in tomorrow]})
            if path == "/api/bookings":
